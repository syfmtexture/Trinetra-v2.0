...
def temporal_attention_rollout(
    ...
    """
    The 'Stress Test' for video frames.
    
    This function looks at the AI's internal thoughts for each frame of a 
    video. It tracks how suspicious the AI is at every millisecond. 
    
    If there's a 'shatter point' (a sudden jump in suspicion), it means the 
    deepfake likely flickered or glitched at that exact moment.
    """
    with torch.no_grad():
...
def noise_residual_extraction(
    ...
    """
    The 'Digital Fingerprint' scanner (Error Level Analysis).
    
    Real photos have a consistent 'noise' across the whole image. Deepfakes 
    often have 'cleaner' or 'different' noise in the face area because that 
    part was generated by an AI.
    
    We intentionally re-save the photo and see what changes. If the face 
    region reacts differently than the background, we've found a 'digital 
    blending scar'.
    """
...
def geometric_jitter_mapping(
    ...
    """
    The 'Lie Detector' for facial movements.
    
    Deepfake faces often 'shimmer' or vibrate slightly because the AI is 
    struggling to keep the face perfectly aligned with the person's head. 
    
    We track 5 key points (eyes, nose, mouth) across the video. If they 
    jitter more than a natural human face would, we flag it as an unstable 
