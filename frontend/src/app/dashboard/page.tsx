...
import { Activity, UploadCloud, FileVideo, FileImage, ShieldCheck, ShieldAlert, ShieldX, Languages, LogOut, Info, ArrowRight, Lock, Loader2, CheckCircle2, XCircle, Wifi, WifiOff, Share2, LockKeyhole, Zap, ArrowLeft, Copy, MessageCircle, ExternalLink, Send, Download, Globe, AlertTriangle, CheckCircle, Mail } from 'lucide-react';
...
export default function GeneralPage() {
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-transparent p-6 text-[#212529]">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="max-w-md w-full bg-white rounded-3xl p-10 shadow-lg border border-[#DEE2E6]"
        >
          <div className="flex justify-between items-center mb-8">
            <div className="h-10">
              <img src="/Trinetra_V3.png" alt="Logo" className="h-full object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
            </div>
            <button onClick={toggleLanguage} className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-[#F1F3F5] text-xs font-bold">
              <Languages className="w-4 h-4 text-[#FF6B00]" />
              <span>{t.language}</span>
            </button>
          </div>

          <h2 className="text-3xl font-bold font-['Syncopate'] text-black mb-2">{t.loginTitle}</h2>
          <p className="text-[#868E96] mb-8">{t.loginSubtitle}</p>

          <div className="space-y-4 mb-6">
            <input type="email" placeholder={t.emailPlaceholder} className="w-full px-4 py-3 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6] text-[#212529] focus:outline-none focus:border-[#FF6B00]" />
            <input type="password" placeholder={t.passwordPlaceholder} className="w-full px-4 py-3 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6] text-[#212529] focus:outline-none focus:border-[#FF6B00]" />
            <button
          ...
          <div className="relative flex items-center py-4">
            <div className="flex-grow border-t border-[#DEE2E6]" />
            <span className="flex-shrink-0 mx-4 text-[#868E96] text-xs">OR</span>
            <div className="flex-grow border-t border-[#DEE2E6]" />
          </div>

          <button
            onClick={() => setIsLoggedIn(true)}
            className="w-full py-3 bg-white border-2 border-[#DEE2E6] text-[#212529] rounded-xl font-bold hover:border-[#FF6B00] hover:text-[#FF6B00] transition-colors flex items-center justify-center space-x-2"
          >
  ...
  // ═══════════════ MAIN DASHBOARD ═══════════════
  return (
    <div className="min-h-screen flex flex-col pt-20 bg-transparent text-[#212529]">
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-[#DEE2E6] flex items-center justify-between px-6 z-50 shadow-sm">
        ...
        <div className="flex items-center space-x-4">
          <button onClick={toggleLanguage} className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-[#F1F3F5] hover:bg-[#E9ECEF] border border-[#DEE2E6] text-sm font-medium transition-colors">
            <Languages className="w-4 h-4 text-[#FF6B00]" />
            <span>{t.language}</span>
          </button>
          <button onClick={() => { setIsLoggedIn(false); setSelectedFile(null); setPreviewUrl(null); setResult(null); }} title={t.logout} className="p-2 rounded-full hover:bg-[#F1F3F5] text-[#868E96] transition-colors">
      ...
      <main className="flex-1 flex flex-col p-6 w-full max-w-[1600px] mx-auto lg:px-12">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl lg:text-5xl font-bold font-['Syncopate'] text-black mb-3">{t.title}</h2>
          <p className="text-[#868E96] text-base lg:text-lg font-medium max-w-2xl mx-auto">{t.subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10">
          ...
          <div className="lg:col-span-7 flex flex-col gap-4">
            <div
              ...
              className={`p-10 rounded-3xl bg-white border-2 border-dashed flex flex-col items-center justify-center text-center cursor-pointer transition-colors min-h-[300px] shadow-sm flex-1 relative overflow-hidden group ${
                selectedFile ? 'border-[#40C057] bg-[#EBFBEE]' : 'border-[#DEE2E6] hover:border-[#FF6B00] hover:bg-[#FFF4E6]'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {!selectedFile ? (
                <>
                  ...
                  <h3 className="text-[#495057] text-xl font-bold mb-1">{t.dropMedia}</h3>
                  <p className="text-sm text-[#868E96] mb-2">{t.orBrowse}</p>
                  <p className="text-xs text-[#ADB5BD]">{t.supported}</p>
                </>
              ) : (
                <div className="flex flex-col items-center">
                  ...
                  <div className="flex items-center space-x-2 text-[#40C057]">
                    ...
                    <span className="text-xs text-[#868E96]">({(selectedFile.size / 1024 / 1024).toFixed(1)} MB)</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold text-lg shadow-md transition-all flex items-center justify-center space-x-3 ${
                !selectedFile || isAnalyzing
                  ? 'bg-[#DEE2E6] text-[#ADB5BD] cursor-not-allowed'
            ...
            <div className={`glass-panel p-8 flex-1 flex flex-col justify-center ${
              ...
              {!result ? (
                <div className="text-center flex flex-col items-center opacity-60">
                  <div className="w-24 h-24 rounded-full border-[6px] border-[#DEE2E6] flex items-center justify-center mb-4">
                    <span className="text-3xl font-bold text-[#DEE2E6] font-['Syncopate']">--</span>
                  </div>
                  <h3 className="text-sm font-bold text-[#868E96] uppercase tracking-wider mb-2">{t.status}</h3>
                  <span className="text-xl font-bold text-[#ADB5BD]">{isAnalyzing ? t.analyzing : t.statusAwaiting}</span>
                </div>
              ) : (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col">
                  ...
                  <div className="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                      <h3 className="text-[10px] font-bold text-[#868E96] uppercase tracking-wider">{t.authenticity}</h3>
                  ...
                  {/* Confidence Score */}
                  <div className="flex items-center justify-between mb-6 p-4 bg-white rounded-xl border border-[#DEE2E6]">
                    <span className="text-sm font-bold text-[#495057]">Confidence</span>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="p-3 rounded-xl bg-white border border-[#DEE2E6]">
                      <span className="text-[10px] font-bold text-[#868E96] uppercase block mb-1">{t.localVerdict}</span>
                    ...
                    <div className="p-3 rounded-xl bg-white border border-[#DEE2E6]">
                      <span className="text-[10px] font-bold text-[#868E96] uppercase block mb-1">{t.rdStatus}</span>
                      <span className={`font-bold text-sm ${result.rd_status === 'DISABLED' ? 'text-[#868E96]' : result.rd_status === 'AUTHENTIC' ? 'text-[#40C057]' : 'text-[#7950F2]'}`}>
                        {result.rd_status || 'N/A'}
                      </span>
                    </div>
                    <div className="col-span-2 p-3 rounded-xl bg-white border border-[#DEE2E6]">
                      <span className="text-[10px] font-bold text-[#868E96] uppercase block mb-1">{t.latency}</span>
                      <span className="font-bold text-sm text-[#212529] font-mono">{result.latency_ms.toFixed(0)} ms</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    ...
                    {isFake && (
                      <>
                        ...
                        <button onClick={() => setActivePanel('takedown')} className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#212529]/10 to-[#495057]/10 border border-[#495057]/30 text-[#212529] text-xs font-bold hover:from-[#212529]/20 hover:to-[#495057]/20 transition-all">
        ...
        {/* ── Evidence Tabs ── */}
        <div className="mt-8 glass-panel overflow-hidden bg-white mb-10">
          <div className="flex flex-wrap border-b border-[#DEE2E6] bg-[#F8F9FA]">
            <button
              onClick={() => setActiveTab('evidence')}
              className={`flex items-center justify-center space-x-2 flex-1 min-w-[200px] py-4 text-sm font-bold transition-colors border-b-[3px] ${activeTab === 'evidence' ? 'border-[#FF6B00] text-[#FF6B00] bg-white' : 'border-transparent text-[#868E96] hover:bg-[#F1F3F5]'}`}
            >
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center space-x-2 min-w-[200px] py-4 text-sm font-bold transition-colors border-b-[3px] ${activeTab === 'details' ? 'border-[#FF6B00] text-[#FF6B00] bg-white' : 'border-transparent text-[#868E96] hover:bg-[#F1F3F5]'}`}
            >
                ...
                <motion.div key="evidence" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col">
                  {!result ? (
                    <div className="flex flex-col items-center justify-center h-full text-center py-10 opacity-50">
                      <ShieldCheck className="w-12 h-12 text-[#ADB5BD] mb-4" />
                    ...
                    <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
                      ...
                      <div className="lg:w-1/4 flex flex-col">
                        <h4 className="text-lg font-bold text-[#212529] mb-3">{t.evidenceTitle}</h4>
                        
                        <div className="mb-4 aspect-square bg-[#F8F9FA] border border-[#DEE2E6] rounded-xl overflow-hidden relative group">
                          {result.face_crop_base64 ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center text-[#ADB5BD]">
                        ...
                        <p className="text-[#868E96] leading-relaxed text-xs mb-4">{t.evidenceDesc}</p>
                        <div className="p-4 mt-auto bg-[#F8F9FA] rounded-xl border border-[#DEE2E6] text-xs text-[#495057]">
                          <strong>Key Finding:</strong> {result.forensic_summary}
                        </div>
                      </div>

                      {/* Middle: Confidence Bar Chart */}
                      <div className="lg:w-1/3 w-full h-[320px] bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-4 flex flex-col relative shadow-sm">
                        <h5 className="text-xs font-bold text-[#868E96] uppercase tracking-wider mb-2">Verdict Confidence Breakdown</h5>
                ...
                <motion.div key="details" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full">
                  {!result ? (
                    ...
                    <div className="max-w-3xl">
                      <h4 className="text-xl font-bold text-[#212529] mb-4">Full Analysis Report</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                          <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-1">Primary Verdict</span>
                          <span className={`font-bold text-lg font-mono ${isFake ? 'text-[#FA5252]' : 'text-[#40C057]'}`}>{result.primary_verdict}</span>
                        </div>
                        <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                          <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-1">Overall Confidence</span>
                          <span className="font-bold text-lg font-mono text-[#212529]">{result.confidence_score.toFixed(2)}%</span>
                        </div>
                        <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                          <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-1">Local Model Result</span>
                          <span className="font-bold text-sm font-mono text-[#212529]">{result.local_label} — {result.local_confidence.toFixed(2)}%</span>
                        </div>
                        <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                          <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-1">Reality Defender Cloud</span>
                          <span className="font-bold text-sm font-mono text-[#212529]">{result.rd_status || 'DISABLED'} {result.rd_score ? `(${(result.rd_score * 100).toFixed(1)}%)` : ''}</span>
                        </div>
                        <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                          <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-1">Processing Latency</span>
                          <span className="font-bold text-sm font-mono text-[#212529]">{result.latency_ms.toFixed(2)} ms</span>
                        </div>
                        <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                          <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-1">Architecture</span>
                          <span className="font-bold text-sm font-mono text-[#212529]">EfficientNet-B4 + BiLSTM</span>
                        </div>
                      </div>
                      <div className="p-4 rounded-xl bg-[#F8F9FA] border border-[#DEE2E6]">
                        <span className="text-xs font-bold text-[#868E96] uppercase tracking-wider block mb-2">Forensic Summary</span>
                        <p className="text-sm text-[#495057] leading-relaxed">{result.forensic_summary}</p>
                      </div>
                    </div>
                  )}
                </motion.div>
      ...
      <AnimatePresence>
        {activePanel === 'blast' && result && (
          <motion.div key="blast" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-3xl shadow-2xl border border-[#DEE2E6]" onClick={(e) => e.stopPropagation()}>
              {/* Header */}
              <div className="sticky top-0 bg-white border-b border-[#DEE2E6] px-8 py-5 rounded-t-3xl z-10">
                <button onClick={() => setActivePanel(null)} className="float-left mr-4 mt-1 text-[#868E96] hover:text-[#212529]"><ArrowLeft className="w-5 h-5" /></button>
                <h2 className="text-2xl font-bold font-['Syncopate'] text-black flex items-center space-x-2">
                  {isFake
              ...
              <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* WhatsApp Broadcast */}
                <div className="bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-[#212529] flex items-center space-x-2"><MessageCircle className="w-4 h-4 text-[#25D366]" /><span>WhatsApp Broadcast</span></h3>
                  ...
                  <div className="bg-white border border-[#DEE2E6] rounded-xl p-4 text-sm text-[#212529] leading-relaxed mb-4 font-mono">
                    {isFake ? (
                      <>
                ...
                <div className="bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-[#212529] flex items-center space-x-2"><Mail className="w-4 h-4 text-[#4C6EF5]" /><span>Email Broadcast</span></h3>
                  ...
                  <div className="bg-white border border-[#DEE2E6] rounded-xl p-4 text-sm text-[#212529] leading-relaxed mb-4 font-mono text-xs">
                    {isFake ? (
                      <>
                  ...
                  <div className="flex gap-2 mb-3">
                    <input
                      ...
                      className="flex-1 px-3 py-2.5 rounded-xl bg-white border border-[#DEE2E6] text-sm text-[#212529] focus:outline-none focus:border-[#4C6EF5]"
                    />
                  </div>
                ...
                <div className="bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-6">
                  <div className="flex items-center space-x-2 mb-3">
                    ...
                    <h3 className="font-bold text-[#212529]">Forensic Summary Card</h3>
                  </div>
                  <p className="text-xs text-[#868E96] mb-4">Attach this verified report card to your message for undeniable proof.</p>
                  <div className="bg-white border border-[#DEE2E6] rounded-xl p-5">
                    <p className="text-[9px] font-bold text-[#868E96] uppercase tracking-widest mb-1">TRINETRA V2 ANALYSIS</p>
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between"><span className="text-[#868E96]">Confidence Score</span><span className={`font-bold font-mono ${isFake ? 'text-[#FA5252]' : 'text-[#40C057]'}`}>{result.confidence_score.toFixed(1)}%</span></div>
                      <div className="flex justify-between"><span className="text-[#868E96]">Detection Engine</span><span className="font-bold font-mono text-[#212529]">EfficientNet-B4 + RD</span></div>
                      <div className="flex justify-between"><span className="text-[#868E96]">Latency</span><span className="font-bold font-mono text-[#212529]">{result.latency_ms.toFixed(0)}ms</span></div>
                    </div>
                    <div className="mt-4 p-3 bg-[#FFF3EC] border border-[#FF6B00]/20 rounded-lg text-xs text-[#495057] italic">&ldquo;{result.forensic_summary}&rdquo;</div>
          ...
          <motion.div key="lockdown" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-3xl shadow-2xl border border-[#DEE2E6]" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="sticky top-0 bg-white border-b border-[#DEE2E6] px-8 py-5 rounded-t-3xl z-10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <button onClick={() => setActivePanel(null)} className="mr-4 text-[#868E96] hover:text-[#212529]"><ArrowLeft className="w-5 h-5" /></button>
                    <div>
                      <h2 className="text-2xl font-bold font-['Syncopate'] text-black flex items-center space-x-2">
                        <LockKeyhole className="w-6 h-6 text-[#FA5252]" /><span>Digital Lockdown Protocol</span>
                      </h2>
                      <p className="text-sm text-[#868E96] mt-1">Tactical crisis management. Follow these steps immediately.</p>
                    </div>
                  </div>
              ...
              <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Immediate Actions */}
                <div className="">
                  <h3 className="font-bold text-[#212529] mb-4 flex items-center space-x-2"><AlertTriangle className="w-4 h-4 text-[#FA5252]" /><span>Immediate Actions</span></h3>
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`w-full flex items-start space-x-3 p-4 rounded-xl border text-left transition-all ${
                          lockdownChecks[i] ? 'bg-[#EBFBEE] border-[#40C057]/30' : 'bg-[#F8F9FA] border-[#DEE2E6] hover:border-[#FF6B00]/40'
                        }`}
                      >
                        ...
                        <div>
                          <span className={`font-bold text-sm ${lockdownChecks[i] ? 'text-[#40C057] line-through' : 'text-[#212529]'}`}>{item.label}</span>
                          {item.link && (
                ...
                {/* Crisis Advisor */}
                <div className="bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-6 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-[#212529] flex items-center space-x-2"><MessageCircle className="w-4 h-4 text-[#7950F2]" /><span>Crisis Advisor</span></h3>
                  ...
                  <div className="flex-1 bg-white border border-[#DEE2E6] rounded-xl p-4 mb-4 min-h-[280px]">
                    <div className="bg-[#F1F3F5] rounded-xl p-4 text-sm text-[#495057] leading-relaxed max-w-[85%]">
                  ...
                  <div className="flex items-center space-x-3">
                    <input type="text" placeholder="Describe your situation..." className="flex-1 px-4 py-3 rounded-xl bg-white border border-[#DEE2E6] text-sm text-[#212529] focus:outline-none focus:border-[#7950F2]" />
      ...
      <AnimatePresence>
        {activePanel === 'takedown' && result && (
          <motion.div key="takedown" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} className="w-full max-w-5xl max-h-[90vh] overflow-y-auto bg-white rounded-3xl shadow-2xl border border-[#DEE2E6]" onClick={(e) => e.stopPropagation()}>
              {/* Header */}
              <div className="sticky top-0 bg-white border-b border-[#DEE2E6] px-8 py-5 rounded-t-3xl z-10">
                <button onClick={() => setActivePanel(null)} className="float-left mr-4 mt-1 text-[#868E96] hover:text-[#212529]"><ArrowLeft className="w-5 h-5" /></button>
                <h2 className="text-2xl font-bold font-['Syncopate'] text-black flex items-center space-x-2">
                  <Zap className="w-6 h-6 text-[#212529]" /><span>SSMI Takedown Engine</span>
                </h2>
                <p className="text-sm text-[#868E96] mt-1">Automated legal compliance strikes under the 2026 IT Rules.</p>
              </div>
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  {[
                    ...
                    <div key={platform.name} className="bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-5">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-bold text-[#212529] flex items-center space-x-2"><span className="text-lg">{platform.icon}</span><span className="text-sm">{platform.name}</span></h3>
                        <span className="text-[8px] font-bold bg-[#212529] text-white px-2 py-0.5 rounded font-mono uppercase">Ready to Fire</span>
                      </div>
                      <p className="text-[10px] font-bold text-[#868E96] uppercase tracking-wider mb-2">Generated Payload Preview</p>
                      <div className="bg-white border border-[#DEE2E6] rounded-xl p-3 text-[11px] font-mono text-[#495057] mb-4 max-h-[180px] overflow-y-auto">
                ...
                <div className="bg-gradient-to-r from-[#FF6B00]/5 to-[#FA5252]/5 border border-[#FF6B00]/20 rounded-2xl p-6 mb-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-bold text-[#212529] flex items-center space-x-2 mb-1"><Globe className="w-5 h-5 text-[#FF6B00]" /><span>Indian Cybercrime Portal</span></h3>
                      <p className="text-sm text-[#868E96]">File an official complaint on the National Cyber Crime Reporting Portal (cybercrime.gov.in)</p>
                    </div>
                    <button
                      onClick={() => {
                ...
                {/* Submission Log */}
                <div className="bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-6">
                  <h3 className="font-bold text-[#212529] flex items-center space-x-2 mb-3"><Globe className="w-4 h-4" /><span>Submission Log</span></h3>
                  <div className="bg-white border border-[#DEE2E6] rounded-xl p-4 min-h-[60px] font-mono text-xs text-[#495057]">
                    {takedownLog.length === 0 ? (
                      <span className="text-[#ADB5BD] italic">Awaiting submissions...</span>
                    ) : (
